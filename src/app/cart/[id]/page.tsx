...
import { createOnlineOrderAction } from "@/app/_Components/CartAction/CartAction";
import { HandleCash } from "@/app/_Components/CartAction/CartAction";
...
export default function CheckoutComponent() {
  ...
  const cartId = Array.isArray(params.id) ? params.id[0] : params.id;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (paymentMethod === "visa") {
        const session = await createOnlineOrderAction(cartId, { shippingAddress: form });
        toast.success("Redirecting to payment ✅");

        window.location.href = session.session.url;
      } else if (paymentMethod === "cash") {
        const res = await HandleCash(cartId, { shippingAddress: form });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {["details", "phone", "city", "postalCode"].map(field => (
            <input
              ...
              placeholder={field === "postalCode" ? "Postal code (optional)" : field.charAt(0).toUpperCase() + field.slice(1)}
            ...
            {[
              ...
              <div
                ...
                className={`flex-1 text-center py-3 rounded-xl cursor-pointer border ${
                  paymentMethod === option.id ? "bg-emerald-600 text-white border-emerald-600" : "bg-white border-slate-300"
                }`}
