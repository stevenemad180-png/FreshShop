...
export default function Providercart({ children, res, wishlist }: ProvidercartProps) {
  const [numberofcart, setnumberofcart] = useState<number>(() => res?.numOfCartItems ?? 0)
  const [numberofWhishlist, setnumberofWhishlist] = useState<number>(() => wishlist?.count ?? 0)

  return (
    <ContextCart.Provider value={{ numberofcart, setnumberofcart, numberofWhishlist, setnumberofWhishlist }}>
      {children}
